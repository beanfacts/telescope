...
int putimage(char *image, Display *display, Visual *visual, Window *window, GC gc)
{
    ...
    int r = (display, *window,gc, ximage, 0, 0, 0, 0, WIDTH, HEIGHT );
    return 0;
}

double convert_timediff(struct timeval *t2, 
                        struct timeval *t1)
{
    double ct1, ct2;
...
{
    if(argc != 3){
        fprintf(stderr,"Please specify the IP address and port.\n");
        exit(0);
    }
    else{
        int win_b_color;
        ...
        if(sockfd == -1)
        {
            fprintf(stderr, "rsocket creation failed...\n");
            exit(0);
        }
        else
            printf("Rsocket created..\n"); 
